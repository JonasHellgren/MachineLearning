...
public class EnvironmentShip {
    ...
    final Map<Integer,Double> DISTANCE_TO_SHIP_MAP = Map.of(0,1000d, 1,2000d);
    ...
    public StepReturnShip step(int state, double action) {
        ...
        return new StepReturnShip(stateNew, IS_TERMINAL,reward);
    }
    ...
    private boolean isHitting(int state, double normalizedAngle) {
        ...
        double distanceProjectile=SPEED_PROJECTILE_MPS
    ...
    private int getStateNew(int state, boolean isHit) {
        return isHit
                ? RandUtils.getRandomIntNumber(0, NOF_STATES -1)
                : state;
    }
